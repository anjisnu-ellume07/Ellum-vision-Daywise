...
class GroqAnalyzer:
    ...
    def __init__(self, api_key):
        ...
        self.cache_file = "grok_cache.json"
        self.cache = self._load_cache()
    
    def _load_cache(self):
        """Load cached Grok responses"""
    ...
    def _format_groq_response(self, text):
        """Format Grok response with proper HTML formatting"""
    ...
    def generate_daily_inverter_summary(self, device_id, date_str, anomaly_data, feature_analysis, state_info=""):
        ...
        # Prepare the prompt for Grok - focused on daily summary
        prompt = f"""
...
**2. Deep Dive: Root Cause Analysis & Component-Level Investigation**
This is the main section. Write a technical paragraph that:
...
"""
        ...
        if cache_key in self.cache:
            print("Using cached Grok response for daily summary")
        ...
        try:
            ...
            response = requests.post(
                ...
                json={
                    ...
                    "messages": [
                        {
                            "role": "system",
                            "content": "You are an expert solar energy system diagnostician providing concise daily executive summaries for technical teams."
                        },
                        {
                            "role": "user",
                            "content": prompt
                    ...
                    "temperature": 0.1,
                    "max_tokens": 700  # Reduced token limit for concise responses
                },
                timeout=60
            )
            ...
            if response.status_code == 200:
                ...
                error_msg = f"Error calling Grok API: {response.status_code} - {response.text}"
                print(error_msg)
                return error_msg
    ...
    **4. Recommended Investigation:** Prescribe the next technical step to confirm the diagnosis (e.g., "Perform on-site IV curve tracing on each string," "Analyze thermal imaging of the power cabinet," "Download event logs for ground fault errors.").
    """
                ...
                daily_summaries[device_id][date_str] = {
                    ...
                    'avg_severity': float(np.mean(date_data['scores'])),
                    'peak_time': peak_time,
                    'trend': trend,
    ...
    def create_severity_gauge(self, severity_score, device_id, date_str):
        """Create a speedometer-style severity gauge"""
        try:
            # Create figure without polar projection first, then add it
            fig = plt.figure(figsize=(6, 4))
            ax = fig.add_subplot(111, projection='polar')
            
            # Normalize severity score to 0-100 scale
            # Handle negative scores by taking absolute value and scaling appropriately
            normalized_score = min(max(abs(severity_score) * 50, 0), 100)  # Scale factor adjusted
            ...
            ax.text(np.pi/2, 0.3, f"{normalized_score:.1f}", 
